...
export async function generateQuotationPdf(quotation: QuotationData) {
  ...
  if (bookingItems.length > 0) {
    ...
    if (transferItems.length > 0) {
      ...
      autoTable(doc, {
        startY: y,
        head: [['Unidad', 'Direccion', 'Ruta', 'Horario', 'Pasajeros', 'Modalidad', 'Costo']],
        body: transferBody,
        margin: { left: margin, right: margin },
        styles: { fontSize: 8, cellPadding: 2.5, textColor: [30, 41, 59] },
  ...
  // ─── POLÍTICAS ───
  if (quotation.tenant?.policies) {
    const policiesLines = doc.splitTextToSize(quotation.tenant.policies, pageWidth - margin * 2);
